...
#define	MAX_ST	200000	/* maximum message string. no picked from thin air */
...
std::vector<double> Oscilloscope::GetOsccilloscopeSignals(bool Ch1, bool Ch2, bool Ch3, bool Ch4){
    char ip_address[50];
    int sockfd;
    char outbuf[MAX_ST];
    char inbuf[MAX_ST];
  ...
  /*strcpy(outbuf,"WFSU SP,0,FP,0,SN,0\n");
  LECROY_TCP_write(sockfd, outbuf);
  printf ("Request to scope:\n%s\n", outbuf);
...
/*
  ...
  myfile << inbuf;
*/
            strcpy(outbuf,"WAIT;C4:INSPECT? \"DUAL\"\n");
                LECROY_TCP_write(sockfd, outbuf);
                printf ("Request to scope:\n%s\n", outbuf);
...
int Oscilloscope::LECROY_TCP_write(int sockfd, char *buf)
{
...
int Oscilloscope::LECROY_TCP_read(int sockfd, char *buf, int len, int allowable_time)
{
        LECROY_TCP_HEADER header;
        char tmpStr[512];
...
int Oscilloscope::LECROY_TCP_connect(char *ip_address, int allowable_delay)
{
...
void Oscilloscope::LECROY_TCP_bored_now(int sig) {
    printf("Well, I got bored waiting to connect, so I'm going to quit. Bye!\n");
    exit(1);
    }

int Oscilloscope::LECROY_TCP_disconnect(int sockfd) {
    if (LECROY_TCP_connected_flag !=TRUE) return -1;
    close (sockfd);
